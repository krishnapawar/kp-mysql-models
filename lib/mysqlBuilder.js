const {
    isLatest,isLimit,isCheck,isTable,isOne,isInt
  } = require('./isCheck');
...
const getKeyValue = (data, setType = "WHERE") => {
  ...
  const sqlConnect = (pool) => {
    // if(isCheck(setDBConnection())) return "kp-mysql-models is not connect database.Please Check You Database Connection!";
