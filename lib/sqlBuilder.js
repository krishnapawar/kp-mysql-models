...
} = require('@krishnapawar/kp-mysql-models/lib/isCheck');
...
const selectOption = (data,x) => {
  if ((data != undefined && data.select != undefined && Array.isArray(x.select) && data.select.length > 0) || (Array.isArray(x._show) && x._show.length > 0))
