...
const whereIn = (data) => {
  ...
  for (const [key, value] of Object.entries(data)) {
    const formattedValues = Array.isArray(value)
      ...
      : `'${value}'`;
    if (setItem.length > 0) {
      setItem.push(` AND ${key} IN (${formattedValues})`);
    } else {
      setItem.push(`${key} IN (${formattedValues})`);
    }
  }
  return ` ${setItem.join(" ")}`;
};
...
const whereNotIn = (data) => {
  ...
  for (const [key, value] of Object.entries(data)) {
    const formattedValues = Array.isArray(value)
      ...
      : `'${value}'`;
    if (setItem.length > 0) {
      setItem.push(` AND ${key} NOT IN (${formattedValues})`);
    } else {
      setItem.push(`${key} NOT IN (${formattedValues})`);
    }
  }
  return `${setItem.join(" ")}`;
};
...
const joinTable = (data) => {
  ...
  if (!Array.isArray(data) && isObject(data) && Array.isArray(data.join)) {
    data = data.join;
...
const getDataWith = async (errThrow, resData, datas, dbcon = null) => {
  let db = dbcon ?? pool;
  ...
  const processKey = async (key, data) => {
    ...
    if (!isOne(data) && (!isCheck(data.pagination) || data.pagination >= 0)) {
      ...
      if (isCheck(data.with)) {
       ...
       return resData[key] = res.length > 0 ? await getDataWith(errThrow, res[0], data, db) : {};
      }
    } else {
      if (isCheck(data.with)) {
        return resData[key] = res;
      } else {
        return resData[key] = await Promise.all(res.map(async (r) => await getDataWith(errThrow, r, data, db)));
      }
    }
  };
