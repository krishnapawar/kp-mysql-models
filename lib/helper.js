...
const {
  isLatest,isLimit,isEmpty,isTable
...
function setDBConnection(db) {
  if (!db || isEmpty(db) || isNotObject(db)) {
...
const create = (data,dbcon=null) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (data.elements != undefined && data.elements != null) {
      const keys = Object.keys(data.elements).join(", ");
      const values = Object.values(data.elements).map(value => `'${value}'`).join(", ");
      db.query(
...
const errorWatcher =async(config)=>{
  const {error="",status=false, table="", con={}} = config;
  if(status){
    console.error("errorWatcher error.code=>",error.code.trim(),"table",table);
    if(error.code && error.code.trim() == "ER_NO_SUCH_TABLE" && table){
      await dbQuery(`CREATE TABLE IF NOT EXISTS ${table} (
        id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
        createdAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updatedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
      )`,con);
      console.error("errorWatcher error.code jjjjjj=>",error.code);
    }
  }
}
const collect =(ar = [])=>{
