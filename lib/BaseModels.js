...
class BaseModels extends QueryBuilder {
  ...
  removeDuplicates(list) {
    ...
    return list.filter(item => {
      const key = `${item.method}-${item.data.table}`;
      if (seen.has(key)) {
        return false;
      }
      seen.add(key);
  ...
  // Method to set the list and remove duplicates
  setList(list) {
  ...
  dbJoin(x={}) {
    return new Promise((resolve, reject) => {
      try {
        ...
        this._connection.query(
          `SELECT ${selectOption(this.x, this)} FROM ${this.thisTable()} ${joinTable(
            this.x.join
          )} ${whereClause(this.x)} ${isLatest(this.x)} ${isLimit(
            this.x
          )} ${pagination(this.x)}`,
          (err, response) => {
            if (err) {
            ...
            if (this.x != undefined && (!isCheck(this.x.pagination) || this.x.pagination >= 0)) {
              this._connection.query(
                `SELECT count(*) as totalData FROM ${this.thisTable()} ${joinTable(
                  this.x.join
                )} ${whereClause(this.x)} ${isLatest(this.x)}`,
                (err, resp) => {
                  if (err) {
                    return reject(err);
                  }
  ...
  thisTable() {
    return isCheck(this._table) ? this.constructor.name.replace(/([A-Z])/g, '_$1').toLowerCase().slice(1)+'s' : this._table;
  }
  exists(x) {
    return new Promise(async (resolve, reject) => {
      try {
  ...
  async setQuery(x, seen = new Set()) {
    ...
    console.log("x=>",x,"tlist",this.#list)
